...
struct AllChart: View {
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            .chartXVisibleDomain(length: 3600 * 24 * 365) // 1 year
            .chartScrollTargetBehavior(
                .valueAligned(
                    matching: DateComponents(month: 1),
                    majorAlignment: .matching(DateComponents(year: 1))
                )
            )
...
struct AllChart_Previews: PreviewProvider {
    static var previews: some View {
        AllChart(chartModel: ChartModel(chartType: .temperature))
