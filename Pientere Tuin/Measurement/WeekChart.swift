...
struct WeekChart: View {
        ...
        VStack(alignment: .leading) {
            ...
            .chartScrollTargetBehavior(
                .valueAligned(
                    ...
                    majorAlignment: .matching(DateComponents(weekOfYear: 1))
                )
            )
    ...
    private func annotationContent(for dayAverage: ChartableMeasurement) -> some View {
        MeasurementAnnotation(
            caption: Formatters.itemFormatter.string(from: dayAverage.date),
            value: dayAverage.value,
            unit: chartModel.valueUnit,
            specifier: chartModel.valueSpecifier
        )
    }
}
...
struct WeekChart_Previews: PreviewProvider {
    static var previews: some View {
        WeekChart(chartModel: ChartModel(chartType: .temperature))
